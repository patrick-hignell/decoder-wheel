...
function App() {
  ...
  const [hideNotKey,setHideNotKey] = useState<boolean>(false)
...
{number: "Z", dice:'Blank'}
  ...
  return (
      ...
      {showOuter && outerWheel.map((dice, i) => (
        ...
        >
            <Dice {...dice} hideNotKey={dice.dice !=key.value.dice || dice.number !=key.value.number ? hideNotKey : false} />
        </div>
        ))}
  ...
  {/* <WheelOutline radius={innerRadius} /> */}
      ...
      <div className="flex flex-col items-center w-full">
        <div className="flex w-full justify-center gap-10">
            <p className="font-['Bellota'] text-6xl mb-4 ml-8">Show outer wheel</p>
            <input className="w-12 h-12" type='checkbox' checked={showOuter} onChange={handleShowOuter} />
            <p className="font-['Bellota'] text-6xl mb-4 ml-8">Show inner wheel</p>
          <input className="w-12 h-12" type='checkbox' checked={showInner} onChange={handleShowInner} />
          <p className="font-['Bellota'] text-6xl mb-4 ml-8">Hide not key</p>
            <input className="w-12 h-12" type='checkbox' checked={hideNotKey} onChange={handleHideNotKey} />
        </div>
